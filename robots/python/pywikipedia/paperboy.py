...
reqs = []
for eachpage in gen:
  reqs.append(eachpage.title(withNamespace=False))
...
if len(reqs) > 0:
  ...
  for subscribername in subscriberlinks:
    pywikibot.output(u"Subscriber: " + subscribername.title(withNamespace=False))
    subscribertalkpage = page.Page(site, u"Discuție Utilizator:" + subscribername.title(withNamespace=False))
    ...
    for req in reqs:
      if tptext.find(u"Andrebot.Newsletter.Notificat:" + req) < 0:
        ...
        pywikibot.output(subscribername.title(withNamespace=false) + u" already knows about " + req)
    if len(newrequests) > 0:
      pywikibot.output(u"Notifying " + subscribername.title(withNamespace=False) + " about " + repr(newrequests))
      ...
      pywikibot.output(subscribername.title(withNamespace=False) + u" already informed")
