...
def extractElectoralDistrict(bf):
	...
	if match:
		print match.groups()
		return parliament.allCommas(" ".join([match.group(1), match.group(2).title(), match.group(3).strip()]))
	else:
		return u""
...
def extractBirthdate(bf):
	...
	if match:
		return u"-".join([match.group(3), parliament.months[match.group(2)], match.group(1)])
	else:
		return u""
...
def scrollThroughPages():
	...
	for camera in [1,2]:
		for om in range(1,500):
			...
			f.write(name + u";" + extractBirthdate(parsed_html) + u";" + str(camera) + u";\"" + extractElectoralDistrict(parsed_html) + u"\"\n")
			count[camera] += 1
	f.close()
...
if __name__ == "__main__":
	...
	for person in people:
		print repr(people[person].generateInfobox())
		pass
