...
from strainu_functions import is_null_or_empty
import csvUtils
...
def log_links_for_page(wiki, level, code, message, page_list=None):
    msg = f"* [{wiki}] ''{level}'': ''[{code}]'' {message}: "
    if not is_null_or_empty(page_list):
        msg += "<span lang=\"x-sic\">"
...
def log_coords_for_page(wiki, level, code, message, coords):
    if len(coords) > 0:
        msg = f"* [{wiki}] ''{level}'': ''[{code}]'' {message}"
        for src in coords:
            msg += u"<br/>[[:%s]] (%f, %f), " % (src, coords[src][0],
                                                  coords[src][1])
        log(msg)
...
def rebuildTemplate(params, skipEmpty=True):
    ...
    for name in getFields(_lang, _db):
        if skipEmpty and name in params and not is_null_or_empty(params[name]):
                ...
                if not fieldsAreEqual(Changes.coord, artLat, page["lat"]) or \
                   not fieldsAreEqual(Changes.coord, artLon, page["long"]):
                    ...
                    if artSrc[5:] not in _differentCoords:
                        _differentCoords[artSrc] = [artLat, artLon]
                    updateCoord = False
            else:
                artLat = page["lat"]
                artLon = page["long"]
...
def log_different_coords(code):
    global _differentCoords
    log_coords_for_page("Cross", "W", code, "Coordonate diferite între",
                        _differentCoords)
...
def get_best_field_for_image(image, fields, type):
    """
...
def chooseImagePicky(files, fields):
    ...
    for field in fields:
        ret[field] = None
    for file in files:
        imageType = get_best_field_for_image(file["name"], fields, Changes.image)
...
def getCodeForMonument(monument):
...
def needDataFromFile(monument):
    ...
    #print(fields)
    return any([is_null_or_empty(monument.get(field)) for field in fields])
...
def checkUpdateArticleField(monument, article, articleText, force, aggresive):
    ...
    if (not useDataFromPage(article, aggresive) or
        is_null_or_empty(article["name"]) or
...
def find_field_by_type(fields, type: Changes) -> str|None:
    for field in fields:
        if fields[field]['code'] == type:
            return field
    return None
...
def main():
    ...
    ran_data = readRan("ro_ran_db.json") if addRan else {}
    other_data = readOtherData(otherFile) if otherFile else {}

    if check_new_monuments:
        checkNewMonuments(other_data, db_json)

    fields = getFields(_lang, _db)
    creatorField = find_field_by_type(fields, Changes.creator) or "Creatori"
    imageField = find_field_by_type(fields, Changes.image) or "Imagine"
    commonsField = find_field_by_type(fields, Changes.commons) or "Commons"
    latField = None
    lonField = None
    for field in fields:
    ...
    for index, monument in enumerate(db_json):
        ...
        code = getCodeForMonument(monument)
        ...
        try:
            if is_null_or_empty(copyright) or force:
            ...
            if code in pages_local:
                ...
                if len(allPages) > 1:
                    log_links_for_page('WP', 'E', code,
                                       'Codul este prezent în mai multe '
                                        'articole pe Wikipedia',
                                       [x['name'] for x in allPages])
                elif len(allPages) == 1:
                    article = allPages[0]
            ...
            if code in files_commons:
                ...
                if len(files_commons[code]) == 1: #exactly one picture
                    ...
                    if pic_author == None and author_list != "":
                        log_links_for_page(
                            'COM', 'E', code,
                            'În lista de imagini sunt trecuți mai multi '
                            'autori', author_list,
                            )
            ...
            if code in categories_commons:
                ...
                if len(categories_commons[code]) > 1:
                    log_links_for_page(
                        'COM', 'E', code,
                        'Codului îi corespund mai multe categorii la Commons',
                        [":commons:"+x["name"] for x in categories_commons[code]]
                        )
        ...
        if useDataFromPage(article, aggresive) and \
            not is_null_or_empty(article["author"]):
        ...
        #add the author(s) extracted from author pages
            ...
            for author in authors_local[code]:
                if authors.find(author[creatorField]) == -1: #we don't already know the author
                    if authors != "":
                        authors = author[creatorField] + ", " + authors
                    else:
                        authors = author[creatorField]
                if author["Copyright"] and int(author["Copyright"]) > last_death:
                    log_links_for_page(
                        'WPListă', 'W', code,
                        f"Nu toți creatorii sunt trecuti în listă. În "
                        f"biografie, [[{author[creatorField]}]] este trecut ca "
                        f"autor",[])
                    last_death = author["Copyright"]
            a2 = monument[creatorField].strip()
        ...
        #try to find the author in external data
        elif code in other_data and not is_null_or_empty(other_data[code].get(creatorField)):
            print(f"{creatorField} from other data")
            authors = old_authors = monument[creatorField].strip()
            ...
            if authors != u"" and authors.find(author) == -1: #we don't already know the author
                ...
                log_links_for_page('Listă', 'W', code,
                       f"În fișierul importat apare autorul {author}, "
                       f"dar lista are autorii", authors)
            if authors != old_authors: # if something changed, update the text
                if force or is_null_or_empty(old_authors):
                    pywikibot.output(authors)
                    articleText, _ = updateTableData(monument["source"], code, creatorField, authors, monument, text=articleText)
        ...
        #next option: an image from the RAN database
            ...
            for site in ran_data[code]:
                if not is_null_or_empty(site[u"Imagine"]):
        ...
        if force:
            ...
            missingFields = []
            for field in fields:
                if is_null_or_empty(monument.get(field)):
                    missingFields.append(field)
        images = chooseImagePicky(imagelist, missingFields)
        for field in images:
            if not is_null_or_empty(images[field]):
        ...
        if aggresive and is_null_or_empty(monument.get(imageField)) and \
            code.find(u"-a-") > -1 and \
            db_json[index+1][getCfg(_lang, _db).get('idField')] == newcode and \
            not is_null_or_empty(db_json[index+1].get(imageField)):
        ...
        if code in categories_commons:
            ...
            if is_null_or_empty(monument.get(commonsField)) or force:
                ...
                log_links_for_page(
                    'COM', 'E', code,
                    'Codului îi corespund mai multe categorii la Commons',
                    ["commons:" + cat["name"], monument["Commons"]]
                )
        ...
        if code in other_data:
            print(f"Other data {other_data[code]}")
        ...
        for (otherLat, otherLong, otherSrc, otherValid, otherFields) in otherCoords:
            ...
            if lat != 0 and \
                ...
                articleText, updated = updateTableData(monument["source"], code, otherFields.get("lat") or latField, str(otherLat), monument,
                                upload = uploadlat, text = articleText, ask = True, source = otherSrc)
                if updated:
                    articleText, updated = updateTableData(monument["source"], code, otherFields.get("lon") or lonField, str(otherLong), monument,
                                upload = True, text = articleText, ask = True, source = otherSrc)
        log_different_coords(code)

        #Other data
        for entry in otherData:
            for field in entry:
                #print entry[field]
                if not is_null_or_empty(entry[field]) and (is_null_or_empty(monument[field]) or force):
