...
class o2wVillageData:
	...
	def setValueToTemplate(self, text, template, param, value, exists):
		pass
	...
	def putCoordOnWiki(self, lang):
		...
		for row in reader:
			...
			pywikibot.output("\n")
			pywikibot.output("Parsing page: %s" % title)
			pywikibot.output("Coords: %d %d %d lat N %d %d %d long E" % (osmLatd, osmLatm, osmLats, osmLongd, osmLongm, osmLongs))
			...
			if code <> None:
				text = self.setValueToTemplate(text, village_templates, "codpoștal", osmCode, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "codpoștal", osmCode, False)
			...
			if latd <> None:
				...
				latd = float(latd)
				if latd <> osmLatd:
					text = self.setValueToTemplate(text, village_templates, "latd", osmLatd, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "latd", osmLatd, False)
			...
			if latm <> None:
				...
				latm = float(latm)
				if latm <> osmLatm:
					text = self.setValueToTemplate(text, village_templates, "latm", osmLatm, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "latm", osmLatm, False)
			...
			if lats <> None:
				...
				lats = float(lats)
				if latds<> osmLats:
					text = self.setValueToTemplate(text, village_templates, "lats", osmLats, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "lats", osmLats, False)
				
			latNS = self.getTemplateParam(village_templates, "latNS", "(N|S)", text)
			if latNS == None:
				text = self.setValueToTemplate(text, village_templates, "latNS", "N", False)
			...
			if longd <> None:
				...
				longd = float(longd)
				if longd <> osmLongd:
					text = self.setValueToTemplate(text, village_templates, "longd", osmLongd, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "longd", osmLongd, False)
			...
			if longm <> None:
				...
				longm = float(longm)
				if longm <> osmLongm:
					text = self.setValueToTemplate(text, village_templates, "longm", osmLongm, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "longm", osmLongm, False)
			...
			if longs <> None:
				...
				longs = float(longs)
				if longs <> osmLongs:
					text = self.setValueToTemplate(text, village_templates, "longs", osmLongs, True)
			else:
				text = self.setValueToTemplate(text, village_templates, "longs", osmLongs, False)
				
			longEV = self.getTemplateParam(village_templates, "longEV", "(E|V)", text)
			if longEV <> None:
				text = self.setValueToTemplate(text, village_templates, "latEV", "E", False)
