...
class w2oWikiLinks:
	...
	def reuploadVillage(self, title, version):
		data = xml.dom.minidom.parseString(self.xmlText)
		data = data.getElementsByTagName("osm")[0].getElementsByTagName("node")[0]
		data = self.api._DomParseNode(data)
		if version == 0: #cheat - we actually have revision current - 1, the node was deleted
			data['version'] += 1 
		else:
			data['version'] = version
		self.api.ChangesetCreate({u"comment": "Add %s" % title.decode('utf8')})
		node = self.api.NodeUpdate(data)
	...
	def addWikiLink(self, node_id, title):
		...
		if node == None:
			hist = self.api.NodeHistory(node_id)
			max = 0
			print hist
			if len(hist) > 0:
				for ver in hist:
					if ver > max:
						max = ver 
			pywikibot.output("Deleted node")
			node = self.reuploadVillage(title, max)
		elif not 'name' in node["tag"]:
			pywikibot.output("Node without name: %s" % node)
			node = self.reuploadVillage(title, node["version"])
		tags = node["tag"]
	...
	def searchByCoord(self, lat, long, name, nameSup = "", county = ""):
		...
		self.xmlText = self.pageText(server, url)
		#print xmlText
		nodeid   = self.extractValueFromOsmData(self.xmlText, "id", "place", None)
		truelat  = self.extractValueFromOsmData(self.xmlText, "lat", "place", None)
		truelong = self.extractValueFromOsmData(self.xmlText, "lon", "place", None)
		truecode = self.extractValueFromOsmData(self.xmlText, "postal_code", "place", None)
		xmlNameSup = self.extractValueFromOsmData(self.xmlText, "siruta:name_sup", "place", None)
		xmlCounty = self.extractValueFromOsmData(self.xmlText, "is_in:county", "place", None)
